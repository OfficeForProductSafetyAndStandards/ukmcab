...
public class RedisCache : IDistCache
{
    ...
    private ConnectionMultiplexer Connection => _connectionMultiplexer ??
                                                throw new InvalidOperationException(
                                                    $"The {nameof(RedisCache)} object has not be initialised.");
    ...
    public T? GetOrCreate<T>(string key, Func<T> action, TimeSpan? expiry = null, Action<T> onCacheItemCreation = null,
        int databaseId = -1)
    {
    ...
    public async Task<T?> GetOrCreateAsync<T>(string? key, Func<Task<T>> action, TimeSpan? expiry = null,
        Func<T, Task>? onCacheItemCreation = null, int databaseId = -1)
    {
        var db = Connection.GetDatabase(databaseId);
        var redisResult = await _retryPolicyAsync
            .ExecuteAsync(async () => await db.StringGetAsync(key).ConfigureAwait(false)).ConfigureAwait(false);
        if (redisResult.HasValue)
        {
            return JsonSerializer.Deserialize<T>(redisResult!);
        }
        ...
        if (result != null)
        {
            await _retryPolicyAsync.ExecuteAsync(async () =>
                    await db.StringSetAsync(key, JsonSerializer.Serialize(result), expiry).ConfigureAwait(false))
    ...
    public void Remove(string[] keys, int databaseId = -1) =>
        Connection.GetDatabase(databaseId).KeyDelete(keys.Select(x => (RedisKey)x).ToArray());

    public void Append(string key, string item, int databaseId = -1) =>
        Connection.GetDatabase(databaseId).StringAppend(key, item);
    ...
    {
        var redisResult =
            await _retryPolicy.Execute(async () => await Connection.GetDatabase(databaseId).StringGetAsync(key));
        return !redisResult.HasValue
            ? default
            : typeof(T) == typeof(string)
                ? (T)Convert.ChangeType(redisResult, typeof(T))
    ...
    public async Task SetAddAsync(string key, string item, int databaseId = -1) => await _retryPolicyAsync.ExecuteAsync(
        async () =>
            await Connection.GetDatabase(databaseId)
                .SetAddAsync(key, item ?? throw new Exception("item cannot be null!")));

    public string[] GetSetMembers(string key, int databaseId = -1) => _retryPolicy.Execute(() =>
        Connection.GetDatabase(databaseId).SetMembers(key)?.Select(x => x.ToString()).ToArray() ??
        Array.Empty<string>());

    public async Task<string[]> GetSetMembersAsync(string key, int databaseId = -1) =>
        await _retryPolicyAsync.ExecuteAsync(async () =>
            (await Connection.GetDatabase(databaseId).SetMembersAsync(key))?.Select(x => x.ToString()).ToArray() ??
            Array.Empty<string>());
    ...
    public bool LockTake(string name, LockOwner lockOwner, TimeSpan duration, int databaseId = -1)
        => Connection.GetDatabase(databaseId)
            .LockTake(CleanKey(name), lockOwner.Id, duration, CommandFlags.DemandMaster);
    ...
    public bool LockExtend(string name, LockOwner lockOwner, TimeSpan duration, int databaseId = -1)
        => Connection.GetDatabase(databaseId)
            .LockExtend(CleanKey(name), lockOwner.Id, duration, CommandFlags.DemandMaster);

    public async Task<bool> LockTakeAsync(string name, LockOwner lockOwner, TimeSpan duration, int databaseId = -1)
        => await Connection.GetDatabase(databaseId)
            .LockTakeAsync(CleanKey(name), lockOwner.Id, duration, CommandFlags.DemandMaster);

    public async Task<bool> LockReleaseAsync(string name, LockOwner lockOwner, int databaseId = -1)
    {
        var key = CleanKey(name);
        var retVal = await Connection.GetDatabase(databaseId)
            .LockReleaseAsync(key, lockOwner.Id, CommandFlags.DemandMaster);
        return true;
    }

    public async Task<bool> LockExtendAsync(string name, LockOwner lockOwner, TimeSpan duration, int databaseId = -1)
        => await Connection.GetDatabase(databaseId)
            .LockExtendAsync(CleanKey(name), lockOwner.Id, duration, CommandFlags.DemandMaster);
    ...
    public void Flush(int databaseId = 0) => Connection.GetEndPoints().ToList()
        .ForEach(x => Connection.GetServer(x).FlushDatabase(databaseId));

    public async Task FlushAsync(int databaseId = 0)
    {
        var tasks = Connection.GetEndPoints().ToList()
            .Select(x => Connection.GetServer(x).FlushDatabaseAsync(databaseId));

...
public class SubscriptionService : ISubscriptionService, IClearable
{
    ...
    private readonly string subscriptionTableKey = $"{SubscriptionsCoreServicesOptions.TableNamePrefix}subscriptions";
    ...
    public async Task<bool> IsSubscribedToSearchAsync(EmailAddress emailAddress, string? searchQueryString) 
        => await _repositories.Subscriptions.ExistsAsync(new SubscriptionKey(subscriptionTableKey, emailAddress, SearchQueryString.Process(searchQueryString, _options))).ConfigureAwait(false);

    public async Task<bool> IsSubscribedToCabAsync(EmailAddress emailAddress, Guid cabId)
        => await _repositories.Subscriptions.ExistsAsync(new SubscriptionKey(subscriptionTableKey, emailAddress, cabId)).ConfigureAwait(false);
    ...
    public async Task<ConfirmSearchSubscriptionResult> ConfirmSearchSubscriptionAsync(string payload)
    {
        ...
        if (validation == ValidationResult.Success)
        {
            ...
            var key = new SubscriptionKey(subscriptionTableKey, options.EmailAddress, cleansedSearchQueryString);
    ...
    public async Task<ConfirmCabSubscriptionResult> ConfirmCabSubscriptionAsync(string payload)
    {
        ...
        if (validation == ValidationResult.Success)
        {
            var key = new SubscriptionKey(subscriptionTableKey, options.EmailAddress, options.CabId);
    ...
    public async Task<SearchResultsChangesModel?> GetSearchResultsChangesAsync(string id)
    {
        ...
        {
            ServiceURL = _options.AwsServiceUrl,
            ForcePathStyle = true
        };
        var blobs = new AmazonS3Client(
            _options.AwsAccessKey,
            _options.AwsSecretKey,
            s3Config
        );

        //var blobs = S3SnapshotsBucket.Create(_options.AwsAccessKeyId, _options.AwsSecretAccessKey, _options.AwsRegion);
        ...
        try
        {
            var response = await blobs.GetObjectAsync(SubscriptionsCoreServicesOptions.BlobContainerPrefix, id).ConfigureAwait(false);

            using var reader = new StreamReader(response.ResponseStream);
            var json = await reader.ReadToEndAsync().ConfigureAwait(false);

            return System.Text.Json.JsonSerializer.Deserialize<SearchResultsChangesModel>(json, new JsonSerializerOptions { WriteIndented = false });
        }

...
{
    public static IHtmlContent Conditional(this IHtmlHelper htmlHelper, bool condition, string text) =>
        condition ? htmlHelper.Raw(text) : HtmlString.Empty;

    public static IHtmlContent Conditional(this IHtmlHelper htmlHelper, bool condition, HtmlString html) =>
        condition ? html : HtmlString.Empty;

    public static IHtmlContent Conditional(this IHtmlHelper htmlHelper, bool condition, string? text,
        string substituteText) => condition ? htmlHelper.Raw(text) : htmlHelper.Raw(substituteText);

    public static IHtmlContent Conditional(this IHtmlHelper htmlHelper, bool condition, HtmlString html,
        HtmlString substituteHTML) => condition ? html : substituteHTML;

    public static IHtmlContent Conditional<T>(this IHtmlHelper<T> htmlHelper, bool condition, string text) =>
        condition ? htmlHelper.Raw(text) : HtmlString.Empty;

    public static IHtmlContent Conditional<T>(this IHtmlHelper<T> htmlHelper, bool condition, HtmlString html) =>
        condition ? html : HtmlString.Empty;

    public static IHtmlContent SanitiseURL(this IHtmlHelper htmlHelper, string? text) =>
        text.StartsWith("http", StringComparison.InvariantCultureIgnoreCase)
            ? htmlHelper.Raw(text)
            : htmlHelper.Raw($"https://{text}");
    ...
    public static IHtmlContent ShowModelStateFormGroupErrorClass(this IHtmlHelper htmlHelper,
    ...
    public static IHtmlContent ShowModelStateInputErrorClass(this IHtmlHelper htmlHelper,
        ModelStateDictionary modelState, string modelStateKey)
    {
        return ShowErrorClass(htmlHelper, modelState, modelStateKey, "govuk-input--error");
    }

    public static IHtmlContent ShowModelStateSelectErrorClass(this IHtmlHelper htmlHelper,
        ModelStateDictionary modelState, string modelStateKey)
    {
        return ShowErrorClass(htmlHelper, modelState, modelStateKey, "govuk-select--error");
    }

    public static IHtmlContent ShowErrorClass(this IHtmlHelper htmlHelper, ModelStateDictionary modelState,
        string modelStateKey, string errorClass)
    {
        return modelState.Keys.Any(k => k.Equals(modelStateKey)) &&
               modelState[modelStateKey].ValidationState == ModelValidationState.Invalid

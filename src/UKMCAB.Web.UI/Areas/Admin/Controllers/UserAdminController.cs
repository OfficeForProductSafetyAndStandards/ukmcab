...
public class UserAdminController : Controller
{
    ...
    {
        var options = new UserAccountListOptions(SkipTake.FromPage(page-1,Constants.RowsPerPage), new SortBy(sortField, sortDirection ?? SortDirectionHelper.Descending), isLocked, lockReason, null);
    ...
    public async Task<IActionResult> AccountRequestList(int pageNumber = 1, [FromQuery(Name = "sf")] string? sortField = null, [FromQuery(Name = "sd")] string? sortDirection = null)
    {
        ...
        var pendingAccounts = await _userService.ListRequestsAsync(new UserAccountRequestListOptions(SkipTake.FromPage(pageNumber - 1, Constants.RowsPerPage), new SortBy(sortField, sortDirection ?? SortDirectionHelper.Descending), UserAccountRequestStatus.Pending));
